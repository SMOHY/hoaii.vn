...
- Nav bar instance ("Frame 494") is **absolutely positioned** at top (`left:0, top:0, width:1920px`), height `168px`, sits on top of/before the banner in stacking — treat as a shared `_Nav` partial with 3 stacked rows:
  ...
  - Utility strip: bg `#F2F2F2`, height 48px, content max-width 1440px centered, `justify-between`; left = icon + "Hơn 100+ mẫu bánh và quà tặng độc đáo" (15px Regular); right = "Về chúng tôi / Liên hệ / Đại lý / Blog" links, gap 40px
